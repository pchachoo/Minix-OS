...
/* Allocate a free inode on 'dev', and return a pointer to it. */
  ...
  if (sp->s_rd_only) {	/* can't allocate an inode on a read only device. */
    ...
    /* ############################# GROUP 1 PROJECT 3 ############################
     // Acquire an inode from the bit map. Check if inode is part of deleted inodes list
    do{
        b = alloc_bit(sp, IMAP, sp->s_isearch);
        inodeSearchReturn = searchOnInode(b);//search hashtable to see if that inode is being saved for a possible file undelete
        if(inodeSearchReturn==true){
            inodeFound = 1;
            free_bit(sp, IMAP, b);//free bit just allocated
        }
        else //inodeSearchReturn = false
            inodeFound = 0;
    } while(inodeFound == 1);//if inode exists in table, alloc a different bit to b
    if(inodeFound == 0)	printf("No match in hashtable");
    if ((inodeFound == 1)&&(b == NO_BIT)) {//all available inodes have been used or preserved; need to free one of the list
        //deleteFromHash();//remove oldest element of the recover list; cant use this as there is no way to call alloc_bit on that specific inode
        b = alloc_bit(sp, IMAP, sp->s_isearch);//let one of the elements of the recover list be used
        deleteFromHashTable(b);//delete that inode from the hashtable
    }
    
    if ((inodeFound == 0)&&(b == NO_BIT)) {//there are no available inodes
        err_code = ENOSPC;
...
struct inode *retrieve_inode(dev_t dev, int inumb){
    ...
    alloc_bit(sp, IMAP, b); // Make a test here ?
    
    /* Acquire the deleted slot in the inode table. */
    if ((rip = get_inode(dev, inumb)) == NULL) {
        /* No inode table slots available.  Free the inode just allocated. */
        free_bit(sp, IMAP, b);
    } else {
        /* An inode slot is available. Put the inode just allocated into it. */
    ...
    for (int i = 0; i < V2_NR_TZONES; i++) {
        printf("zone[%d] = %d", i, rip->i_zone[i]);
    }
