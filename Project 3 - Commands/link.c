  ...
  if (r == OK) {
    ...
    /* ######################### GROUP 1 PROJECT 3 ################################
     ...
     if (getcwd(curDir, sizeof(curDir)) != NULL){
        fullPath = combineStrings(curDir, file_name);
        insertToHash(rip->i_num, fullPath);
     }
     else {
...
/*===========================================================================*
 *				System call to retrieve an inode, added for CS551		     *
 *===========================================================================*/
...
int fs_recover(){
    ...
    printf("Entering System call recover: success\n");
    //printf("inode received = %ld \n", fs_m_in.REQ_INODE_NR);
    ...
    if ((ldirp = get_inode(fs_m_in.REQ_DEV, (ino_t) fs_m_in.REQ_DIR_INO)) == NULL)
        return(ENOENT);
    
    //printf("inode dir = %d \n", ldirp->i_num);
    
    
    rip = retrieve_inode(fs_m_in.REQ_DEV, fs_m_in.REQ_INODE_NR);
    rip->i_nlinks++; // Added to the parent directory
    IN_MARKCLEAN(rip);
    
    //printf("mode : %d \t", rip->i_mode);
    //printf("links : %d \t", rip->i_nlinks);
    //printf("dev : %d \n", rip->i_dev);
    
    /* Copy the file name */
    // Used to retrieve the file name to delete if m_in.m1_p1 was working
    ...
    search_dir(ldirp, "test.c", &rip->i_num, ENTER, IGN_PERM); //Replace 'test.c' by the filename called lastc here
